...
**Author (Role): `r params$name` (`r params$role `)**

```{r  echo=FALSE,warning=FALSE }
# "<br><span style='font-weight:bold'>Author (Role): </span> `r input$name` (`r input$role `)"
setwd(params$cwd)
...
comment_o <-
  db_fun(
    paste0(
      ...
      params$package,"'", # ac: specify version in this table?
      " AND comment_type = 'o'"
    )
  )
  ...
  if (!(params$package %in% pkgs_in_db$cum_id)) {
    ...
    riskmetrics_cum <- db_fun( paste0("SELECT * FROM CommunityUsageMetrics WHERE cum_id ='", params$package, "'") ) # ac: specify version in this table?
  } else{
    riskmetrics_cum <- db_fun( paste0("SELECT * FROM CommunityUsageMetrics WHERE cum_id ='", params$package, "'") ) # ac: specify version in this table?
  }
